...
DATA_FILE = Path(__file__).resolve().parents[1] / "data" / "cards.json"
...
_add("Colorless", [
    ...
    # This historical maintenance map can override source values, so
...
def main(dry_run: bool | None = None, data_path: Path | None = None):
    if dry_run is None:
        dry_run = "--dry-run" in sys.argv

    with open(data_path or DATA_FILE, "r", encoding="utf-8") as f:
        cards = json.load(f)
    ...
    if not dry_run and changes:
        with open(data_path or DATA_FILE, "w", encoding="utf-8") as f:
