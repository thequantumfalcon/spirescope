...
- Review move sets and turn patterns for the 102 Act 1 to Act 3 pool monsters
...
- **The coverage badge is generated by CI instead of hand-typed.** It was a
  static shields.io badge that only changed when someone remembered to change
  it, so it had drifted from the figure CI actually measured — the one claim
  on the front page that could be falsified by running the suite. CI now
  writes `coverage.json` beside `tests.json` from pytest-cov's own total, and
